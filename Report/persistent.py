...
root_dir, option, with_pts, without_pts, choice = '', - \
    1, [], [], ('with_pts', 'without_pts')
    ...
    for i in range(0, data_length, 6):
        print('{0:<{length}}\t{1:<{length}}\t{2:<{length}}\t{3:<{length}}\t{4:<{length}}\t{5:<{length}}'
              .format(data[i] if i < data_length else '', data[i + 1] if i + 1 < data_length else '',
                      data[i + 2] if i + 2 < data_length else '', data[i +
                                                                       3] if i + 3 < data_length else '',
                      data[i + 4] if i + 4 < data_length else '', data[i +
                                                                       5] if i + 5 < data_length else '',
                      length=8))
