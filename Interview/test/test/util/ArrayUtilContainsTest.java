...
public class ArrayUtilContainsTest {
	...
	public void testContainsGenericArray() {
		...
		Boolean[] bNone = null;
		Boolean[] bEmpty = {};
		Boolean[] bActual = ArrayUtil.toObject(bg.generate(20));
		boolean bvalueToFind = bActual[2];
		
		assertFalse(ArrayUtil.contains(bNone, bvalueToFind));
		assertFalse(ArrayUtil.contains(bEmpty, bvalueToFind));
		assertTrue(ArrayUtil.contains(bActual, bvalueToFind));
		...
		System.out.println(Arrays.toString(actualInteger));
		System.out.println(intValueToFind);
