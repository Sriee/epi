...
public class Flatten {
    ...
    /**
     * Leet code. Solution -> Accepted Run Time: 0 ms. Optimal Solution Flatten a
     * Doubly Linked List with child pointer. Example: 1 -> 2 -> 3 -> 4 -> 5 -> null
     * | v 6 -> 7 -> 8 -> null | v 9 -> 10 -> 11 -> null Returns: 1 -> 2 -> 6 -> 7
     * -> 8 -> 9 -> 10 -> 11 -> 3 -> 4 -> 5 -> null
     * 
    ...
    private <T> void print(DoubleListNode<T> root) {
        ...
        while (current != null) {
            System.out.println(((current.prev != null) ? current.prev.val : 0) + " " + current.val + " "
                    + ((current.next != null) ? current.next.val : 0));
            current = current.next;
        }
    }
