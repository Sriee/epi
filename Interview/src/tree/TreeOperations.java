...
public class TreeOperations {
    ...
    /**
     * Given a Binary Tree check whether its subtree's are symmetric Leet code.
     * Solution -> Accepted
     * 
    ...
    /**
     * Given a Binary Tree check whether its subtree's are symmetric (Iterative
     * implementation) Leet code. Solution -> Accepted Run Time: 1ms Optimal
     * solution is the recursive version
     * 
     * @param root Root of the tree
     * @return true if the sub tree's are symmetric, false otherwise
     */
    ...
    /**
     * Given a Binary Tree check whether the subtree's are same Leet code. Solution
     * -> Accepted
     *
     * @param root Root of the tree
    ...
    public BalanceStatus checkBalanced(TreeNode node) {
        ...
        return new BalanceStatus(Math.abs(leftChild.height - rightChild.height) <= 1,
                1 + Math.max(leftChild.height, rightChild.height));
    }

    /**
     * Given a Binary Tree check is it is Balanced. A Tree is balanced if the
     * diff(leftSubtree.height - rightSubtree.height) <= 1. Leet Code. Solution ->
     * Accepted Note: The fastest run time solution in leet code uses a global
     * boolean variable to store the result. Also in that approach you will be
     * visiting O(n) nodes for both best & worst case scenarios, whereas in the
     * following approach you will be visiting O(h) for best case and O(n) for worst
     * case scenario. This is because, while traversing if any of the subtree is not
    ...
    /**
     * Leet code. Solution -> Accepted Run Time: 9 ms. Above Average run time.
     * Optimal time is recursive approach Maximum Level Sum of Binary Tree
     * 
    ...
    /**
     * Merge tree's Conditions: - If there are nodes in the same position for both
     * the tree's; the merged node should contain the sum value of both nodes - If a
     * node is missing in one of the tree's the merged tree should contain the node
     * which is present Leet code. Solution -> Accepted
     * 
    ...
    private boolean pathDFS(TreeNode node, int sum, int soFar) {
        if (node == null)
            return false;
        soFar += node.val;
        return node.left == node.right ? soFar == sum
                : this.pathDFS(node.left, sum, soFar) || this.pathDFS(node.right, sum, soFar);
    }

    /**
     * Given a binary tree and a sum, determine if the tree has a root-to-leaf path
     * such that adding up all the values along the path equals the given sum. Leet
     * code. Solution -> accepted
     *
     ...
     * numbers represented by the path from the root to that leaf. Leet code.
     * Solution -> accepted
     * 
     * @param root root node of a tree
    ...
    /**
     * Invert a binary tree Leet code. Solution -> Accepted
     * 
    ...
    /**
     * Find the sum of all the left leaves in a binary tree Leet code. Solution ->
     * Accepted
     * 
    ...
    /**
     * Given a binary tree, flatten it to a linked list in-place. Example: 1 / 3 / 5
     * should be flattened as 1 \ 3 \ 5
     * 
     * @param root of the binary tree
     */
    ...
    /**
     * Size of the tree Leet code. Solution -> Accepted
     * 
