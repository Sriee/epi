...
public class GraphProblems {
    ...
    private boolean dagHasCycle(Vertex v, Set<Vertex> beingVisited) {
        /*
         * Why do we need beingVisited set? Why can't we just rely on visited flag
         * alone? Note the location where we are setting the visited flag - Once we have
         * visited all the neighbors for a vertex. If there is a loop, the stack will
         * crash. Normally we would use a variable in vertex class itself such as: -1 -
         * unvisited 0 - being visited 1 - visited instead of an set.
         */
        beingVisited.add(v);
    ...
    /**
     * Detect cycle in a directed graph Why is there a wrapper ? Why can't we use
     * the above method ? If the graph is disconnected, only calling hasCycle on a
     * single vertex wouldn't work
     * 
     * @param g
     * @return
     */
